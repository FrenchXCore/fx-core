	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				...
				balance := suite.app.BankKeeper.GetBalance(suite.ctx, suite.signer.Address().Bytes(), fxtypes.DefaultDenom)

				err = suite.app.BankKeeper.SendCoinsFromModuleToAccount(suite.ctx, types.ModuleName, suite.signer.Address().Bytes(), amt)
				suite.Require().NoError(err)

				expAccount = &types.QueryAccountResponse{
					Balance:  balance.Amount.Add(amt.AmountOf(fxtypes.DefaultDenom)).String(),
					CodeHash: common.BytesToHash(crypto.Keccak256(nil)).Hex(),
					Nonce:    0,
				}
				req = &types.QueryAccountRequest{
					Address: suite.signer.Address().String(),
				}
			},
			true,
		},
...
func (suite *KeeperTestSuite) TestQueryCosmosAccount() {
	...
	testCases := []struct {
		...
		},
		{
			"success",
			func() {
				account := suite.app.AccountKeeper.GetAccount(suite.ctx, suite.signer.Address().Bytes())
				expAccount = &types.QueryCosmosAccountResponse{
					CosmosAddress: sdk.AccAddress(suite.signer.Address().Bytes()).String(),
		...
		},
		{
			...
			func() {
				acc := suite.app.AccountKeeper.GetAccount(suite.ctx, suite.signer.Address().Bytes())
...
func (suite *KeeperTestSuite) TestQueryBalance() {
	...
	testCases := []struct {
		...
		},
		{
			"success",
			func() {
				amt := sdk.Coins{sdk.NewCoin(fxtypes.DefaultDenom, sdk.NewInt(100))}
				err := suite.app.BankKeeper.MintCoins(suite.ctx, types.ModuleName, amt)
				suite.Require().NoError(err)
				...
				expBalance = balance.Amount.Add(amt.AmountOf(fxtypes.DefaultDenom)).String()
				req = &types.QueryBalanceRequest{
					Address: suite.signer.Address().String(),
				}
			},
	...
	testCases := []struct {
		...
		},
		{
			...
			func(vmdb vm.StateDB) {
				...
				vmdb.SetState(suite.signer.Address(), key, value)
				req = &types.QueryStorageRequest{
					Address: suite.signer.Address().String(),
					Key:     key.String(),
				}
...
func (suite *KeeperTestSuite) TestQueryCode() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func(vmdb vm.StateDB) {
				expCode = []byte("code")
				vmdb.SetCode(suite.signer.Address(), expCode)
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				...
				suite.Require().NoError(err)
				account := suite.app.AccountKeeper.GetAccount(suite.ctx, vals[0].GetOperator().Bytes())
				expAccount = &types.QueryValidatorAccountResponse{
					AccountAddress: sdk.AccAddress(vals[0].GetOperator()).String(),
					Sequence:       account.GetSequence(),
					AccountNumber:  account.GetAccountNumber(),
				}
				req = &types.QueryValidatorAccountRequest{
		...
		},
		{
			...
			func() {
				...
				acc := suite.app.AccountKeeper.GetAccount(suite.ctx, vals[0].GetOperator().Bytes())
				suite.Require().NoError(acc.SetSequence(10))
				suite.Require().NoError(acc.SetAccountNumber(1))
				suite.app.AccountKeeper.SetAccount(suite.ctx, acc)
...
func (suite *KeeperTestSuite) TestEstimateGas() {
	...
	testCases := []struct {
		...
		// estimate gas of an erc20 contract deployment, the exact gas number is checked with geth
		{
			...
			func() {
				addr := suite.signer.Address()
				ctorArgs, err := fxtypes.GetFIP20().ABI.Pack("")
				suite.Require().NoError(err)
				data := append(fxtypes.GetFIP20().Bin, ctorArgs...)
				args = types.TransactionArgs{
					From: &addr,
		...
		// estimate gas of an erc20 transfer, the exact gas number is checked with geth
		{
			...
			func() {
				...
				suite.Commit()
				transferData, err := fxtypes.GetFIP20().ABI.Pack("transfer", helpers.GenerateAddress(), big.NewInt(1000))
				suite.Require().NoError(err)
		...
		},
		{
			...
			func() {
				...
				contractAddr := suite.DeployERC20Contract()
				suite.Commit()
				transferData, err := fxtypes.GetFIP20().ABI.Pack("transfer", common.Address{0x1}, big.NewInt(1000))
				suite.Require().NoError(err)
				args = types.TransactionArgs{To: &contractAddr, From: &addr, Data: (*hexutil.Bytes)(&transferData)}
			},
			true,
...
// 				vmdb := suite.StateDB()
// 				vmdb.SetNonce(suite.signer.Address(), vmdb.GetNonce(suite.signer.Address())+1)
// 				suite.Require().NoError(vmdb.Commit())
//
// 				contractAddr := suite.DeployERC20Contract()
// 				suite.Commit()
// 				// Generate token transfer transaction
// 				firstTx := suite.TransferERC20Token(suite.T(), contractAddr, suite.signer.Address(), common.HexToAddress("0x378c50D9264C63F3F92B806d4ee56E9D86FfB3Ec"), sdkmath.NewIntWithDecimal(1, 18).BigInt())
// 				txMsg = suite.TransferERC20Token(suite.T(), contractAddr, suite.signer.Address(), common.HexToAddress("0x378c50D9264C63F3F92B806d4ee56E9D86FfB3Ec"), sdkmath.NewIntWithDecimal(1, 18).BigInt())
// 				suite.Commit()
//
...
// 			suite.SetupTest()
// 			// Deploy contract
// 			contractAddr := suite.DeployTestContract(suite.T(), suite.signer.Address(), sdkmath.NewIntWithDecimal(1000, 18).BigInt())
// 			suite.Commit()
// 			// Generate token transfer transaction
// 			txMsg = suite.TransferERC20Token(suite.T(), contractAddr, suite.signer.Address(), common.HexToAddress("0x378c50D9264C63F3F92B806d4ee56E9D86FfB3Ec"), sdkmath.NewIntWithDecimal(1, 18).BigInt())
// 			suite.Commit()
...
// 				firstTx := suite.TransferERC20Token(suite.T(), contractAddr, suite.signer.Address(), common.HexToAddress("0x378c50D9264C63F3F92B806d4ee56E9D86FfB3Ec"), sdkmath.NewIntWithDecimal(1, 18).BigInt())
// 				secondTx := suite.TransferERC20Token(suite.T(), contractAddr, suite.signer.Address(), common.HexToAddress("0x378c50D9264C63F3F92B806d4ee56E9D86FfB3Ec"), sdkmath.NewIntWithDecimal(1, 18).BigInt())
// 				suite.Commit()
...
// 	address := common.BytesToAddress(helpers.NewEthPrivKey().PubKey().Address().Bytes())
...
// 	ctorArgs, err := fxtypes.GetFIP20().ABI.Pack("", address, supply)
// 	suite.Require().NoError(err)
//
// 	data := append(fxtypes.GetFIP20().Bin, ctorArgs...)
...
func (suite *KeeperTestSuite) TestQueryBaseFee() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			func() {
				expRes = &types.QueryBaseFeeResponse{BaseFee: &feemarkettypes.DefaultBaseFee}
			},
			true,
		},
		{
