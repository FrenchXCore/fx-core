...
func (suite *MsgsTestSuite) SetupTest() {
	privFrom := helpers.NewEthPrivKey()

	suite.signer = helpers.NewSigner(privFrom)
	suite.from = common.BytesToAddress(privFrom.PubKey().Address().Bytes())
	suite.to = helpers.GenerateAddress()
...
func (suite *MsgsTestSuite) TestMsgEthereumTx_BuildTx() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			tx, err := tc.msg.BuildTx(suite.clientCtx.TxConfig.NewTxBuilder(), fxtypes.DefaultDenom)
			if tc.expError {
				...
				suite.Require().Equal(sdk.NewCoins(sdk.NewCoin(fxtypes.DefaultDenom, sdkmath.NewInt(100000))), tx.GetFee())
			}
		})
	}
}
