...
func (suite *BackendTestSuite) TestTraceTransaction() {
	...
	tx, _ := msgEthereumTx.BuildTx(suite.backend.clientCtx.TxConfig.NewTxBuilder(), fxtypes.DefaultDenom)
	txBz, _ := txEncoder(tx)

	msgEthereumTx2.From = from.String()
	_ = msgEthereumTx2.Sign(ethSigner, suite.signer)
	tx2, _ := msgEthereumTx.BuildTx(suite.backend.clientCtx.TxConfig.NewTxBuilder(), fxtypes.DefaultDenom)
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				queryClient := suite.backend.queryClient.QueryClient.(*mocks.EVMQueryClient)
				client := suite.backend.clientCtx.Client.(*mocks.Client)
				_, err := RegisterBlockMultipleTxs(client, 1, []tmtypes.Tx{txBz, txBz2})
				suite.Require().NoError(err)
				RegisterTraceTransactionWithPredecessors(queryClient, msgEthereumTx, []*evmtypes.MsgEthereumTx{msgEthereumTx})
			},
			&tmtypes.Block{Header: tmtypes.Header{Height: 1, ChainID: fxtypes.ChainId()}, Data: tmtypes.Data{Txs: []tmtypes.Tx{txBz, txBz2}}},
			[]*abci.ResponseDeliverTx{
				{
					Code: 0,
					Events: []abci.Event{
...
func (suite *BackendTestSuite) TestTraceBlock() {
	...
	emptyBlock.ChainID = fxtypes.ChainId()
	filledBlock := tmtypes.MakeBlock(1, []tmtypes.Tx{bz}, nil, nil)
	filledBlock.ChainID = fxtypes.ChainId()
