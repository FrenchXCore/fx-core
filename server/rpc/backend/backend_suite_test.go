...
func (suite *BackendTestSuite) SetupTest() {
	...
	ctx.Logger = server.ZeroLogWrapper{Logger: log.Logger.Level(zerolog.Disabled)}
	...
	suite.acc = sdk.AccAddress(helpers.GenerateAddress().Bytes())
...
func (suite *BackendTestSuite) signAndEncodeEthTx(msgEthereumTx *evmtypes.MsgEthereumTx) []byte {
	priv := helpers.NewEthPrivKey()
	signer := helpers.NewSigner(priv)

	queryClient := suite.backend.queryClient.QueryClient.(*mocks.EVMQueryClient)
	RegisterParamsWithoutHeader(queryClient, 1)

	ethSigner := ethtypes.LatestSigner(suite.backend.ChainConfig())
	msgEthereumTx.From = common.BytesToAddress(priv.PubKey().Address().Bytes()).String()
