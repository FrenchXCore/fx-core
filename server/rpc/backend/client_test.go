...
func TestRegisterBlock(t *testing.T) {
	...
	_, err := RegisterBlock(cli, height, nil)
	require.NoError(t, err)

	res, err := cli.Block(rpc.ContextWithHeight(height), &height)
	require.NoError(t, err)
...
func TestRegisterConsensusParams(t *testing.T) {
	cli := mocks.NewClient(t)
	height := int64(1)
	RegisterConsensusParams(cli, height)

	res, err := cli.ConsensusParams(rpc.ContextWithHeight(height), &height)
	consensusParams := types.DefaultConsensusParams()
...
func TestRegisterBlockResults(t *testing.T) {
	cli := mocks.NewClient(t)
	height := int64(1)
	_, err := RegisterBlockResults(cli, height)
	require.NoError(t, err)

	res, err := cli.BlockResults(rpc.ContextWithHeight(height), &height)
	require.NoError(t, err)
