...
func (suite *BackendTestSuite) TestResend() {
	...
	toAddr := helpers.GenerateAddress()
...
func (suite *BackendTestSuite) TestSendRawTransaction() {
	...
	cosmosTx, _ := ethTx.BuildTx(suite.backend.clientCtx.TxConfig.NewTxBuilder(), fxtypes.DefaultDenom)
...
func (suite *BackendTestSuite) TestGasPrice() {
	defaultGasPrice := (*hexutil.Big)(feemarkettypes.DefaultMinGasPrice.TruncateInt().BigInt())
